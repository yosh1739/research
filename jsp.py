...
def ComputeDAG(s, I):
    ...
    for t in s: G.append([])
    G.append([])
    T = [0 for j in range(I.n)]
    last_task_job = [-1 for j in range(I.n)]
    tasks_resource = [[-1 for j in range(I.n)] for m in range(I.m)]
    ...
    for i in range(len(s)):
        ...
        G[i].extend([tasks_resource[r][j2] for j2 in range(I.n)
...
def FormatSolution(s, C, I):
    T = [0 for j in range(I.n)]
    S = [[0 for t in I[j]] for j in range(I.n)]
    for i in range(len(s)):
        j = s[i]
        t = T[j]
        S[j][t] = C[i]
...
def Genetic(I, ps = PS, pc = CP, pm = MP, mit = IT):
    def InitPopulation(ps, I):
        ...
        gene = [j for j in range(I.n) for t in I[j]]
        population = []
        for i in range(ps):
            shuffle(gene)
